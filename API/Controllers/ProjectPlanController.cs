...
namespace API.Controllers
{
    ...
    public class ProjectPlanController : ControllerBase
    {
        ...
        [HttpPost("{employeeId}")]
        public async Task<IActionResult> Post([FromBody] ProjectPlanInsert projectPlanForInsert, int employeeId)
        {
            var projectPlan = _mapper.Map<ProjectPlan>(projectPlanForInsert);
            if (!await _logic.Insert(projectPlan))
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] ProjectPlanForUpdate projectPlanForUpdate)
        {
